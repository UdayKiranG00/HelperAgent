...
export default async function mainFn(userQuery: string, streamWriter): string {
  try {
    ...
    while (taskItems.length > 0) {
      output.write(
        `In Facade fn planned tasks are: ${JSON.stringify(taskItems)}.`,
      );
      for (let task of taskItems) {
        ...
        output.write(`In Facade fn task status: ${taskResult.status}`);
      ...
      if (planAgain) {
        if (mainTaskRetries >= 2)
          throw new Error(
            `Unable to execute task given, chat history: ${taskHistory.toString()}`,
          ); // stop after 3 tries returning task history.
        taskItems = await planTasksBySchema(
    ...
    output.write(
      `run agent task Final tasks output: ${taskHistory.toString()}`,
    );
    return taskHistory.toString();
  } catch (error) {
    throw new Error(`failed execution due to ${error}`);
  }
}
